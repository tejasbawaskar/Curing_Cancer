...
#variable introduced to overcome infeasibility
...
minimize dosage : sum {(m,n) in critical_area} sum {i in MATS} X[i] * target_value[i,m,n] + sum {(m,n) in critical_area} B[m,n] + sum {(m,n) in tumor_area} A[m,n] - sum {(m,n) in tumor_area} sum {i in MATS} X[i] * target_value[i,m,n];
